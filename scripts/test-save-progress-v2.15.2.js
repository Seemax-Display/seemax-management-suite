...
(async () => {
  ...
  const quotation = sandbox.startQuotationProgress({ id_preventivo: '151-26', cliente: 'Cliente Test' });
  check(elements.saveProgressItems.innerHTML.includes('Preventivo'), 'Salvataggio preventivo non visibile.');
  check(elements.saveProgressItems.innerHTML.includes('151-26'), 'Numero preventivo non visibile.');
  sandbox.window.SeemaxBackgroundOperations.update(quotation, 'Registrazione nell’archivio…');
  check(elements.saveProgressSubtitle.textContent.includes('Registrazione'), 'Fase preventivo non aggiornata.');
  sandbox.window.SeemaxBackgroundOperations.complete(quotation, { id_preventivo: '151-26', cliente: 'Cliente Test' });

  const upload = sandbox.startDocumentUploadProgress({ id: 'DOC-2', nome: 'Manuale.pdf' });
  check(elements.saveProgressItems.innerHTML.includes('Documento'), 'Upload manuale documento non visibile.');
  check(elements.saveProgressItems.innerHTML.includes('Manuale.pdf'), 'Nome documento non visibile.');
  sandbox.window.SeemaxBackgroundOperations.fail(upload, { id: 'DOC-2', nome: 'Manuale.pdf' }, 'Drive non disponibile');
  check(elements.saveProgressItems.innerHTML.includes('Da verificare'), 'Errore upload manuale non segnalato.');

  for (const item of sandbox.saveProgressState.items.values()) clearInterval(item.timer);
  console.log(`Test operazioni UI 2.15.2 OK: avanzamento ${firstProgress}%, salvataggi, preventivi, documenti, eliminazioni, magazzino, concorrenza ed errori.`);
})().catch((error) => {
  for (const item of sandbox.saveProgressState.items.values()) clearInterval(item.timer);
  console.error(error);
  process.exit(1);

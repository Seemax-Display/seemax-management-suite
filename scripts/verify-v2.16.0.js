...
check(/version:\s*["']2\.16\.0["']/.test(files.config), 'config frontend 2.16.0');
check(/SEEMAX_VERSION\s*=\s*["']seemax-management-suite-2\.16\.0["']/.test(files.backend), 'backend 2.16.0');
check(/seemax-management-v2-16-0-release-ready/.test(files.sw), 'cache PWA 2.16.0');
check(/upgradeSeemaxV2160\s*\(/.test(files.backend), 'funzione upgradeSeemaxV2160 presente');
...
check(/setupSeemaxDatabase\(\).*upgradeSeemaxV2160/s.test(files.backend.slice(0, 1000)), 'istruzioni distinguono nuovo database e upgrade');
...
check(/rebuildQuoteCountersV2151_\(\);/.test(files.backend.slice(files.backend.indexOf('function upgradeSeemaxV2160'), files.backend.indexOf('function removeEmptyLegacyActivitySheetV2130'))), 'upgrade inizializza contatori preventivi');
...
// Preventivi e upload manuali in background, preservati nel rilascio 2.16.0.
...
console.log(`Verifica statica e funzionale 2.16.0 completata senza errori: ${checks} controlli.`);

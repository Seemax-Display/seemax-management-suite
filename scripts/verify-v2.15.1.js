...
check(/version:\s*["']2\.15\.1["']/.test(files.config), 'config frontend 2.15.1');
check(/SEEMAX_VERSION\s*=\s*["']seemax-management-suite-2\.15\.1["']/.test(files.backend), 'backend 2.15.1');
check(/seemax-management-v2-15-1/.test(files.sw), 'cache PWA 2.15.1');
check(/upgradeSeemaxV2151\s*\(/.test(files.backend), 'funzione upgradeSeemaxV2151 presente');
...
check(/setupSeemaxDatabase\(\).*upgradeSeemaxV2151/s.test(files.backend.slice(0, 1000)), 'istruzioni distinguono nuovo database e upgrade');
...
console.log(`Verifica statica e funzionale 2.15.1 completata senza errori: ${checks} controlli.`);

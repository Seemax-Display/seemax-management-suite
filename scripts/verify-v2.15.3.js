...
check(/version:\s*["']2\.15\.3["']/.test(files.config), 'config frontend 2.15.3');
check(/SEEMAX_VERSION\s*=\s*["']seemax-management-suite-2\.15\.3["']/.test(files.backend), 'backend 2.15.3');
check(/seemax-management-v2-15-3/.test(files.sw), 'cache PWA 2.15.3');
check(/upgradeSeemaxV2153\s*\(/.test(files.backend), 'funzione upgradeSeemaxV2153 presente');
...
check(/setupSeemaxDatabase\(\).*upgradeSeemaxV2153/s.test(files.backend.slice(0, 1000)), 'istruzioni distinguono nuovo database e upgrade');
...
check(/function resetAgentProfilesForLaunchV2153_/.test(files.backend), 'migrazione reset profili presente');
check(/reset_profili_v2153_eseguito/.test(files.backend), 'migrazione reset profili idempotente');
...
check(/rebuildQuoteCountersV2151_\(\);/.test(files.backend.slice(files.backend.indexOf('function upgradeSeemaxV2153'), files.backend.indexOf('function removeEmptyLegacyActivitySheetV2130'))), 'upgrade inizializza contatori preventivi');
...
// Preventivi e upload manuali in background 2.15.3.
...
console.log(`Verifica statica e funzionale 2.15.3 completata senza errori: ${checks} controlli.`);

...
check(/version:\s*["']2\.15\.2["']/.test(files.config), 'config frontend 2.15.2');
check(/SEEMAX_VERSION\s*=\s*["']seemax-management-suite-2\.15\.2["']/.test(files.backend), 'backend 2.15.2');
check(/seemax-management-v2-15-2/.test(files.sw), 'cache PWA 2.15.2');
check(/upgradeSeemaxV2152\s*\(/.test(files.backend), 'funzione upgradeSeemaxV2152 presente');
...
check(/setupSeemaxDatabase\(\).*upgradeSeemaxV2152/s.test(files.backend.slice(0, 1000)), 'istruzioni distinguono nuovo database e upgrade');
...
check(/rebuildQuoteCountersV2151_\(\);/.test(files.backend.slice(files.backend.indexOf('function upgradeSeemaxV2152'), files.backend.indexOf('function removeEmptyLegacyActivitySheetV2130'))), 'upgrade inizializza contatori preventivi');
...
// Preventivi e upload manuali in background 2.15.2.
...
console.log(`Verifica statica e funzionale 2.15.2 completata senza errori: ${checks} controlli.`);

...
check(/version:\s*["']2\.16\.1["']/.test(files.config), 'config frontend 2.16.1');
check(/SEEMAX_VERSION\s*=\s*["']seemax-management-suite-2\.16\.1["']/.test(files.backend), 'backend 2.16.1');
check(/seemax-management-v2-16-1-quotation-admin/.test(files.sw), 'cache PWA 2.16.1');
check(/upgradeSeemaxV2161\s*\(/.test(files.backend), 'funzione upgradeSeemaxV2161 presente');
...
check(/setupSeemaxDatabase\(\).*upgradeSeemaxV2161/s.test(files.backend.slice(0, 1000)), 'istruzioni distinguono nuovo database e upgrade');

// Archivio preventivi amministrativo.
check(/admin_view:\s*adminView/.test(files.backend), 'backend dichiara esplicitamente la vista ADMIN');
check(/item\.agent_username/.test(files.backend) && /item\.agent_display_name/.test(files.backend), 'lista ADMIN include autore e username');
check(/if \(adminView && foreignQuote\)/.test(files.backend), 'payload alternativo limitato ad ADMIN su preventivo altrui');
check(/authorized_payload_json/.test(files.backend) && /response\.payload_criptato = row\.payload_criptato/.test(files.backend), 'risposta minima separa payload ADMIN e payload cifrato personale');
check(/\["action", "requestId", "agent_key"\]/.test(files.backend), 'chiave account esclusa dalla persistenza preventivo');
check(/function clearArchivedQuoteKeysV2161_/.test(files.backend) && /range\.clearContent\(\)/.test(files.backend), 'chiavi archiviate eliminate dall’upgrade');
check(/saved_by_login[\s\S]{0,120}Preventivo protetto da account/.test(files.backend), 'endpoint pubblico blocca i preventivi autenticati');
check(/function loadQuotePublic_[\s\S]{0,800}payload_criptato:[\s\S]{0,400}save_request_token/.test(files.backend), 'endpoint pubblico restituisce soltanto il payload cifrato necessario');
check(/if\(record\.authorized_payload_json\)/.test(files.planner), 'Planner usa il payload autorizzato per l’apertura ADMIN');
check(/id="adminQuoteAgentFilter"/.test(files.planner), 'filtro agente ADMIN presente');
check(/id="adminQuoteSortBy"/.test(files.planner) && /Data creazione/.test(files.planner), 'ordinamento per data e agente presente');
check(/Creato da:/.test(files.planner) && /agentQuoteArchiveState\.adminView/.test(files.planner), 'autore mostrato soltanto nella vista ADMIN');
check(/listquotes:\s*true/.test(files.backend) && /loadquote_agent:\s*true/.test(files.backend) && /deletequote_agent:\s*true/.test(files.backend), 'azioni archivio ammesse sul ponte persistente');
check(/async function listQuotations\(/.test(files.api) && /async function loadQuotation\(/.test(files.api) && /async function deleteQuotation\(/.test(files.api), 'API nativa archivio preventivi completa');
check(/listQuotations:\s*\(\)\s*=>\s*window\.SeemaxApi\.listQuotations/.test(files.plannerNative), 'elenco preventivi collegato al runtime nativo');
check(/loadQuotation:\s*\(id\)\s*=>\s*window\.SeemaxApi\.loadQuotation/.test(files.plannerNative), 'caricamento preventivo collegato al runtime nativo');
check(/deleteQuotation:\s*\(id\)\s*=>\s*window\.SeemaxApi\.deleteQuotation/.test(files.plannerNative), 'eliminazione preventivo collegata al runtime nativo');
check(/typeof nativeApi\.listQuotations === ['"]function['"]/.test(files.planner) && /typeof nativeApi\.loadQuotation === ['"]function['"]/.test(files.planner), 'Planner preferisce il ponte per elenco e caricamento');
check(/jsonpRequest\(['"]listquotes['"]/.test(files.planner) && /jsonpRequest\(['"]loadquote_agent['"]/.test(files.planner), 'fallback standalone archivio preservato');
...
check(/contentChanged[\s\S]*publishPatchNow = republish \|\| contentChanged/.test(files.backend), 'una modifica patch genera automaticamente una nuova pubblicazione');
check(/Salva e pubblica/.test(files.app), 'editor patch espone un unico comando di pubblicazione esplicito');
check(/showPatchNotesMessage\(true\)/.test(files.app), 'admin riceve conferma visiva immediata della patch pubblicata');
check(/data-action="acknowledge-message"/.test(files.app), 'la lettura delle comunicazioni richiede conferma esplicita');
check(/function acknowledgeMessage\([\s\S]*markMessageSeen/.test(files.app), 'la revisione viene marcata soltanto dal comando di conferma');
...
check(/unlockAllTrophies:\s*false/.test(files.config), 'sblocco globale trofei disattivato');
check(/function betaTrophiesUnlocked\(\)\s*\{\s*return api\.isAdmin\(\)/.test(files.app), 'tutti i trofei restano disponibili soltanto per ADMIN');
check(/"trofei_reset_il"/.test(files.backend), 'colonna punto di partenza trofei presente');
check(/function resetAgentProfilesForOfficialReleaseV2160_/.test(files.backend), 'migrazione reset profili ufficiale presente');
check(/reset_profili_v2160_eseguito/.test(files.backend), 'migrazione reset profili ufficiale idempotente');
check(/function cleanupUnusedSheetsForReleaseV2160_/.test(files.backend), 'pulizia sicura fogli presente');
check(/makeCopy\(ss\.getName\(\) \+ ["'] - BACKUP PRE-RILASCIO 2\.16\.0/.test(files.backend), 'backup completo prima della pulizia');
check(/function auditSheetStructureV2160\s*\(/.test(files.backend), 'audit struttura fogli presente');
check(/SEEMAX_VISIBLE_SHEETS_V2160_/.test(files.backend) && /sheet\.hideSheet\(\)/.test(files.backend), 'tabelle tecniche attive nascoste');
check(files.app.includes('☷ IN DETTAGLIO') && files.app.includes('▦ PER TIPOLOGIA'), 'nuove diciture modalità pratiche presenti');
check(!/Referente non (?:indicato|assegnato)/i.test(files.app), 'dicitura referente non assegnato eliminata');
check(/const practiceCountByClient = new Map\(\)/.test(files.app), 'conteggi cliente indicizzati in un unico passaggio');
check(/const statusCounts = Object\.fromEntries/.test(files.app) && /const typeCounts = \{ ACQUISTO: 0, NOLEGGIO: 0, LEASING: 0 \}/.test(files.app), 'conteggi pratiche lineari');
check(/BOOTSTRAP_CACHE_PREFIX \+ String\(config\.version/.test(files.api), 'cache bootstrap isolata per versione');
check(/function settingsCacheKey_\(\) \{ return ["']SEEMAX_SETTINGS_["'] \+ SEEMAX_VERSION; \}/.test(files.backend), 'cache impostazioni backend isolata per versione');
...
check(/backgroundSaveNotice = \[["']clients["'], ["']practices["'], ["']documents["']\]/.test(files.app), 'feedback non bloccante esteso a clienti, pratiche e documenti');
...
check(/rebuildQuoteCountersV2151_\(\);/.test(files.backend.slice(files.backend.indexOf('function upgradeSeemaxV2161'), files.backend.indexOf('function resetAgentProfilesForLaunchV2153_'))), 'upgrade inizializza contatori preventivi');
...
// Preventivi e upload manuali in background, preservati nel rilascio 2.16.1.
check(/async function\s+saveQuotation\s*\(/.test(files.api), 'API salvataggio preventivo condivisa presente');
check(/postMutation\(["']savequote["']/.test(files.api), 'preventivo usa la mutazione confermata');
check(/savequote:\s*true/.test(files.backend), 'savequote consentito sul ponte nativo');
check(/saveQuotation:\s*\(fields\)\s*=>\s*window\.SeemaxApi\.saveQuotation/.test(read('assets/js/planner-native.js')), 'Planner nativo salva tramite API della suite');
check(/nativeApi[\s\S]*typeof nativeApi\.saveQuotation === ["']function["'][\s\S]*nativeApi\.saveQuotation\(fields\)/.test(files.planner), 'Planner preferisce il salvataggio nativo');
check(/function\s+startQuotationProgress\s*\(/.test(files.app) && /startQuotationBackgroundProgress/.test(files.planner), 'pannello preventivo non bloccante collegato');
check(/completeQuotationBackgroundProgress\(progressId/.test(files.planner) && /failQuotationBackgroundProgress\(progressId/.test(files.planner), 'preventivo conclude il pannello soltanto su esito o errore');
check(/function\s+saveQuotation_\s*\([\s\S]{0,180}withMutationLock_/.test(files.backend), 'preventivo usa il lock globale condiviso');
check(/existing[\s\S]{0,180}save_request_token[\s\S]{0,220}duplicate:\s*true/.test(files.backend), 'retry preventivo idempotente senza seconda scrittura');
check(/function\s+startDocumentUploadProgress\s*\(/.test(files.app), 'upload manuale usa il pannello non bloccante');
check(/Trasferimento del file a Google Drive/.test(files.api) && /Registrazione del documento nel database/.test(files.api), 'fasi reali upload comunicate dalla API');
check(/delete visiblePending\.file_base64/.test(files.app), 'Base64 escluso dallo stato ottimistico del documento');
check(/recordSyncBadge\(d\)/.test(files.app) && /Caricamento in corso/.test(files.app), 'documento in trasferimento riconoscibile e protetto nella lista');
check(/hasActiveDatabaseOperations/.test(files.app) && /beforeunload/.test(files.app), 'uscita protetta durante operazioni in background');
check(/ledFaceLabel\(item\)\.toUpperCase\(\)/.test(files.planner), 'nomenclatura PDF usa faccia maiuscola');
check(/\['Prev\. n\. ' \+ quoteNumberLabel\(\), 'del ' \+ quoteDateFile\(\), clientHeading, ledName, measure, face\]/.test(files.planner), 'nome PDF segue il formato richiesto');
...
check(/action["'],\s*["']management_bridge["']/.test(files.api) || /set\(["']action["'],\s*["']management_bridge["']\)/.test(files.api), 'frontend apre il ponte Apps Script');
check(/function\s+managementBridgeOutput_\s*\(/.test(files.backend), 'pagina ponte backend presente');
check(/function\s+managementBridgeRpc\s*\(/.test(files.backend), 'RPC Apps Script presente');
check(/SEEMAX_BRIDGE_ACTIONS_/.test(files.backend) && /Azione non consentita dal ponte nativo/.test(files.backend), 'azioni del ponte limitate da whitelist');
check(/allowedFrontendOrigin_/.test(files.backend) && /https:\/\/seemax-display\.github\.io/.test(files.backend), 'origine frontend verificata dal backend');
check(/event\.source\s*!==\s*managementBridge\.iframe\.contentWindow/.test(files.api), 'origine finestra del ponte verificata dal frontend');
check(/BRIDGE_VERSION_MISMATCH/.test(files.api), 'versione ponte verificata');
check(/native_bridge_status_recovery/.test(files.api) && /postMutationLegacy/.test(files.api), 'fallback idempotente del ponte presente');
check(/bridge_wait_ms/.test(files.api) && /bridge_fallback/.test(files.api), 'diagnostica ponte presente');
check(/onPending/.test(files.api) && /__sync_state:\s*["']PENDING["']/.test(files.api), 'callback ottimistica API presente');
check(/recordSyncBadge/.test(files.app) && /markLocalEntitySyncFailed/.test(files.app), 'stati sincronizzazione UI presenti');
check(/__sync_new_record/.test(files.app) && /request_token/.test(files.app), 'riconciliazione nuovi record tramite token presente');
check(/record-sync-badge/.test(files.css) && /record-sync-pending/.test(files.css), 'indicatori sincronizzazione stilizzati');
check(/EMAIL_CODA:\s*\[/.test(files.backend), 'schema coda email presente');
check(/function\s+processEmailQueueV2151\s*\(/.test(files.backend), 'worker email asincrono presente');
check(/newTrigger\(["']processEmailQueueV2151["']\)/.test(files.backend), 'trigger email automatico presente');
check(count(files.backend, 'ensureEmailQueueTriggerV2151_();') >= 2, 'trigger email installato per setup e upgrade');
check(/queueDeferredEmailsLocked_\(\);[\s\S]{0,500}SpreadsheetApp\.flush\(\)/.test(files.backend), 'email accodata prima del flush principale');
check(!/function\s+flushDeferredEmails_\s*\(/.test(files.backend), 'invio email sincrono rimosso');
check(/logLockWaitMs:\s*40/.test(files.backend), 'log best-effort non trattiene la risposta');
check(/findRowObject_\(["']MOVIMENTI_MAGAZZINO["'],\s*["']request_token["']/.test(files.backend), 'duplicati movimenti cercati sulla sola colonna chiave');
check(/findRowObject_\(["']PRATICHE["'],\s*["']clientId["']/.test(files.backend), 'vincolo cliente cercato sulla sola colonna chiave');
check(/createTextFinder\(String\(user\.username/.test(files.backend) && /getRangeList\(unreadRows/.test(files.backend), 'notifiche aggiornate con ricerca e scrittura mirate');
const touchLoginBody = (files.backend.match(/function\s+touchLoginBestEffort_\s*\([\s\S]*?\n\}/) || [''])[0];
check(/setValue\(new Date\(\)\.toISOString\(\)\)/.test(touchLoginBody) && !/getScriptLock/.test(touchLoginBody), 'ultimo accesso scritto senza lock globale');
...
equal(runtimeNotes.publication_revision, 8, 'la modifica patch incrementa automaticamente la revisione pubblica');
check(runtimeNotes.publication_key !== 'patch-old', 'la modifica patch cambia automaticamente la chiave pubblica');
...
equal(runtimeNotes.publication_revision, 9, 'ripubblicazione incrementa nuovamente la revisione patch');
...
sandbox.getKeyValueSheet_ = () => ({ publication_revision: 9, publication_key: runtimeNotes.publication_key, version: '9' });
...
console.log(`Verifica statica e funzionale 2.16.1 completata senza errori: ${checks} controlli.`);

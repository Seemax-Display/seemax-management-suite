...
check(/SEEMAX_VERSION\s*=\s*"seemax-management-suite-2\.14\.1"/.test(backend), "Backend versione 2.14.1");
check(/version:\s*"2\.14\.1"/.test(config), "Frontend versione 2.14.1");
check(/seemax-management-v2-14-1/.test(worker), "Cache PWA versione 2.14.1");
check(/function\s+upgradeSeemaxV2141\s*\(/.test(backend), "Funzione upgradeSeemaxV2141 presente");
check(/function\s+updatePatchNotesV2141_\s*\(/.test(backend), "Patch notes 2.14.1 presenti");
...
check(/POST_MESSAGE_GRACE_MS\s*=\s*2600/.test(api), "Fallback POST ridotto da 22 secondi a 2,6 secondi");
...
process.stdout.write("\nVerifica statica e funzionale 2.14.1 completata senza errori.\n");

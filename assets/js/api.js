(function () {
  ...
  async function remoteUpsert(entity, record) {
    ...
    if (response.notification) row.__notification = response.notification;
    return row;
  }
  ...
  async function remove(entity, id, expectedRecordVersion = 0) {
    ...
    if (isFastMode()) {
      queueOperation({ type: "remove", entity, id, expectedRecordVersion, requestToken: uid("remove") });
      return { ok: true, queued: true };
    }
    if (config.demoMode) return demo.remove(entity, id);
    const requestToken = uid("remove");
    return postMutation("management_remove", { entity, id, expected_record_version: expectedRecordVersion, request_token: requestToken }, { requestToken, maxWait: 60000, skipFallback: true });
  }
  ...
  async function saveSettings(values) {
    if (isFastMode()) {
      const requestToken = uid("settings-save");
      const queuedValues = { ...(values || {}) };
      queueOperation({ type: "settings", values: queuedValues, requestToken });
      return queuedValues;
    }
    if (config.demoMode) return demo.settings(values);
    const requestToken = uid("settings-save");
    const payload = { ...(values || {}) };
    const response = await postMutation("management_save_settings", { payload: JSON.stringify(payload), request_token: requestToken }, { requestToken, maxWait: 90000 });
    return response.settings;
  }
  ...
  async function acknowledgeAnnouncement(type, revision) {
    ...
    const requestToken = uid("announcement-ack");
  ...
  async function saveProfile(values) {
    ...
    const requestToken = uid("profile-save");
    payload.request_token = requestToken;
    const response = await postMutation("management_save_profile", { payload: JSON.stringify(payload) }, { requestToken, maxWait: 60000 });
  ...
  async function syncAll(onProgress) {
    ...
    for (let index = 0; index < queue.length; index += 1) {
      ...
      if (op.type === "upsert") result = await remoteUpsert(op.entity, op.record);
        ...
        const requestToken = op.requestToken || uid("remove");
        result = config.demoMode ? demo.remove(op.entity, op.id) : await postMutation("management_remove", { entity: op.entity, id: op.id, expected_record_version: op.expectedRecordVersion || 0, request_token: requestToken }, { requestToken, maxWait: 60000, skipFallback: true });
      }
      else if (op.type === "settings") {
        const requestToken = op.requestToken || uid("settings-save");
        const values = { ...(op.values || {}) };
        result = config.demoMode ? demo.settings(values) : await postMutation("management_save_settings", { payload: JSON.stringify(values), request_token: requestToken }, { requestToken, maxWait: 90000 });
      }
      results.push(result);
  ...
  async function adjustInventory(values) {
    ...
    /* Il backend rilegge già la stessa riga di PRODOTTI_LED prima di
       confermare il movimento: una seconda lista completa sarebbe soltanto
       latenza aggiuntiva quando il catalogo cresce. */
  ...
  async function markNotificationsRead() {
    ...
    const response = await postMutation("management_mark_notifications_read", { request_token: requestToken }, { requestToken, maxWait: 60000 });
